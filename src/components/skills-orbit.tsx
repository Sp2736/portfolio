...
export function SkillsOrbit() {
  ...
  // Math to calculate orbital positions based on number of active items
  ...
  return (
    <section className="w-full py-12 px-6 relative z-10 bg-background overflow-hidden" id="skills">
      <div className="max-w-6xl mx-auto flex flex-col lg:flex-row gap-12 items-center">
        
        {/* Left Side: Text & Filters */}
        <div className="lg:w-1/3 flex flex-col gap-6" data-code="<SkillOrbit filter={activeCategory} />">
          <div>
        ...
        <div className="lg:w-2/3 w-full flex justify-center relative min-h-[500px] items-center">
          ...
          {/* Faint background orbital rings */}
          <div className="absolute w-[300px] h-[300px] md:w-[400px] md:h-[400px] rounded-full border border-border/40" />
          <div className="absolute w-[200px] h-[200px] md:w-[260px] md:h-[260px] rounded-full border border-border/20" />

          {/* Central Power Source */}
          <div className="absolute z-20 flex items-center justify-center w-24 h-24 md:w-32 md:h-32 rounded-full bg-card border border-border shadow-[0_0_50px_rgba(var(--primary),0.2)] backdrop-blur-md">
          ...
          <div className="relative w-full h-full flex items-center justify-center">
            <AnimatePresence>
              {filteredSkills.map((skill, index) => {
                // Determine responsive radius based on screen size (rough estimation)
                ...
                return (
                  <motion.div
                    key={skill.name}
                    layout // This is the magic that makes them smoothly slide to new positions when array changes
                  ...
                  >
                    {/* The Logo Container */}
                    <div className="flex items-center justify-center w-12 h-12 md:w-16 md:h-16 rounded-full bg-white/5 backdrop-blur-xl border border-white/10 hover:border-primary hover:bg-primary/10 transition-colors shadow-lg shadow-black/20">
                      <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-white p-1.5 flex items-center justify-center">
                    ...
                    <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50">
                      <div className="px-2 py-1 bg-card border border-border rounded text-[10px] font-mono text-foreground whitespace-nowrap shadow-xl">
                        {skill.name}
                      </div>
                    </div>
                  </motion.div>
