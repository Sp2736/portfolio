...
// Foreground Interactive Cursor (Trails & Sparkles)
...
export function WarpDriveEasterEgg() {
  const [warpState, setWarpState] = useState<"idle" | "spooling" | "warped">(
    "idle",
  );
  const canvasRef = useRef<HTMLCanvasElement>(null);
      ...
      */}
      ...
      {warpState !== "idle" && (
        <div className="fixed top-24 right-10 z-[300] font-mono text-right pointer-events-none transition-opacity duration-500">
          <div
            className={`text-xs tracking-widest uppercase font-black px-2 py-1 inline-block mb-1 border ${
              warpState === "warped"
          ...
          <div
            className={`text-6xl font-black italic leading-none drop-shadow-[0_0_15px_rgba(59,130,246,0.8)] ${
              warpState === "warped" ? "text-cyan-300" : "text-white"
            }`}
          >
            {velocity.toFixed(3)}
            <span className="text-2xl ml-1 text-blue-500">c</span>
          </div>
        </div>
      )}
