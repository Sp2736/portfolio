...
import { Terminal, Home, Cpu, Layers, BookOpen } from "lucide-react";
    ...
    <>
      ...
      >
        <div className="max-w-7xl mx-auto flex items-center justify-between pointer-events-auto">
          <a href="#hero" onClick={(e) => handleNav(e, "#hero")} className="flex items-center gap-2 font-mono font-bold text-lg tracking-tighter hover:opacity-80 transition-opacity">
          ...
          <nav className="hidden md:flex items-center gap-8 font-mono text-sm font-semibold text-muted-foreground">
            {navLinks.map((link) => (
              <a key={link.name} href={link.id} onClick={(e) => handleNav(e, link.id)} className="hover:text-primary transition-colors">
                ./{link.name}
              </a>
            ))}
          </nav>
      ...
      >
        <a href="#hero" onClick={(e) => handleNav(e, "#hero")} className="p-2 rounded-full hover:bg-primary/20 hover:text-primary transition-colors group relative">
          <Terminal size={20} />
          <span className="absolute right-12 top-1/2 -translate-y-1/2 px-2 py-1 bg-background border border-border rounded text-[10px] font-mono opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">Home</span>
        </a>
        <div className="w-full h-[1px] bg-border"></div>
        {navLinks.map((link) => {
          const Icon = link.icon;
          return (
            <a key={link.name} href={link.id} onClick={(e) => handleNav(e, link.id)} className="p-2 rounded-full text-muted-foreground hover:bg-primary/20 hover:text-primary transition-colors group relative">
              <Icon size={20} />
