...
import { 
  Layers, BarChart, Shield, Scan, Puzzle, BrainCircuit, 
  Network, Users, ClipboardList, MessageSquare, 
  Headphones, Activity, Compass, PenTool, Coffee 
} from "lucide-react";
...
const coreStrengths = [
  ...
  { name: "Professional Comm.", icon: MessageSquare },
];
      ...
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-8 lg:gap-12">
        ...
        >
          ...
          <div className="flex flex-col gap-2 border-l-2 border-primary pl-4">
            <h2 className="text-primary font-mono text-[10px] tracking-[0.2em] uppercase">Processing Core</h2>
            <h3 className="text-2xl md:text-3xl font-extrabold text-foreground tracking-tight">Cognitive Architecture</h3>
          </div>
          ...
          <div className="flex flex-col gap-3">
            <h4 className="text-[10px] font-mono text-muted-foreground uppercase tracking-widest flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-primary animate-pulse" /> Primary Directives
            </h4>
              ...
              {focusAreas.map((area, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-[10px] md:text-xs font-bold leading-tight text-foreground">{area.name}</span>
                  </motion.div>
                );
              })}
            </div>
          ...
          <div className="flex flex-col gap-3 mt-2">
            <h4 className="text-[10px] font-mono text-muted-foreground uppercase tracking-widest flex items-center gap-2">
              <span className="w-2 h-2 rounded-full border border-primary" /> Behavioral Metrics
            </h4>
            <div className="flex flex-wrap gap-2 md:gap-3">
              {coreStrengths.map((strength, i) => {
                ...
                return (
                  ...
                  >
                    <Icon size={14} className="text-primary opacity-70 group-hover:opacity-100 group-hover:scale-110 transition-all" />
        ...
        >
          {/* Header */}
          <div className="flex flex-col gap-2 border-l-2 border-primary/50 pl-4 lg:text-right lg:border-l-0 lg:border-r-2 lg:pr-4 lg:items-end">
            <h2 className="text-primary font-mono text-[10px] tracking-[0.2em] uppercase">Beyond The Terminal</h2>
            <h3 className="text-2xl md:text-3xl font-extrabold text-foreground tracking-tight">Organic Subroutines</h3>
          </div>

          {/* THE TIMELINE: Fiber Optic Logic Tree */}
          <div className="relative flex flex-col gap-6 pl-6 lg:pl-10 mt-2">
            {/* Continuous Vertical Glowing Line */}
            <div className="absolute left-[11px] lg:left-[19px] top-2 bottom-2 w-[2px] bg-gradient-to-b from-primary/50 via-primary/20 to-transparent" />

            {organicInterests.map((interest, i) => {
              const Icon = interest.icon;
              return (
                <motion.div key={i} variants={itemVars} className="relative flex flex-col gap-2 group">
                  
                  {/* Timeline Node (The dot on the line) */}
                  <div className="absolute -left-[32px] lg:-left-[40px] top-0.5 w-6 h-6 rounded-full bg-background border border-primary flex items-center justify-center z-10 group-hover:bg-primary/20 transition-colors shadow-[0_0_10px_rgba(var(--primary),0.3)]">
